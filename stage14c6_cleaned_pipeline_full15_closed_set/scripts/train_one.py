#!/usr/bin/env python3
"""Train one frozen Stage 14C-6 cleaned F2 Known-only protocol.

The optimization primitives are imported directly from the immutable Stage
14C-4 implementation.  This wrapper only generalizes the protocol choice and
redirects outputs into the isolated Stage 14C-6 bundle.
"""
...
FROZEN_TRAIN_SOURCE = PROJECT / "stage14c4_our_method_training_cleanup/scripts/train_cleaned.py"
EXPECTED_FREEZE = "c904daef04d2c8e79469191121325c0a6ceeaeb3e595ef9c4c7d7a62c980ae2f"
EXPECTED_TRAIN_SOURCE_SHA256 = "a72b0e52be8c10917c057b49fea58a228253a867007e7390097c27e98cab8655"
PROTOCOLS = tuple(
...
def load_frozen_training_module():
    if sha256_file(FROZEN_TRAIN_SOURCE) != EXPECTED_TRAIN_SOURCE_SHA256:
        raise RuntimeError("immutable Stage 14C-4 training source hash changed")
    spec = importlib.util.spec_from_file_location("stage14c4_frozen_training", FROZEN_TRAIN_SOURCE)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load immutable Stage 14C-4 training source")
...
def write_diagnostics(
    ...
    confusion_path = ROOT / "confusion_matrices" / f"{protocol_id}.csv"
    ...
    accuracy, macro_f1, weighted_f1 = frozen_impl.metrics_from_confusion(confusion)
...
def main() -> None:
    ...
    frozen_impl = load_frozen_training_module()

    frozen = frozen_impl.f2_common.verify_frozen_inputs()
    if frozen["freeze_hash"] != EXPECTED_FREEZE:
        raise RuntimeError("Stage 14B freeze hash changed")
    protocol = frozen_impl.f2_common.stage14c_common.load_protocol(args.protocol_id)
    ...
    if input_audit["status"] != "PASS":
        raise RuntimeError("F2 input audit is not PASS")
    if input_audit["unknown_samples_loaded"] != 0 or input_audit["known_test_samples_loaded"] != 0:
    ...
    run_dir = ROOT / "runs" / args.protocol_id
    best_path = ROOT / "checkpoints" / f"{args.protocol_id}_best.pt"
    latest_path = run_dir / "latest_checkpoint.pt"
    if run_dir.exists() or best_path.exists():
        raise RuntimeError(f"refusing to overwrite evidence for {args.protocol_id}")
    run_dir.mkdir(parents=True)
    best_path.parent.mkdir(parents=True, exist_ok=True)
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA required")

    frozen_impl.f2_common.stage14c_common.seed_everything(training_seed)
    device = torch.device("cuda:0")
    train_ds = frozen_impl.KnownDataset(input_dir, "train", augment=True)
    val_ds = frozen_impl.KnownDataset(input_dir, "validation", augment=False)
    ...
    model = frozen_impl.make_model(len(protocol["known_applications"]), device)
    model.apply(frozen_impl.released_weight_init)
    ...
    config = {
        "method": "F2 own-method cleaned training frozen at Stage 14C-5",
        ...
        "frozen_stage14c4_training_source": str(FROZEN_TRAIN_SOURCE),
        "frozen_stage14c4_training_source_sha256": EXPECTED_TRAIN_SOURCE_SHA256,
        "epochs": 100,
        "batch_size": 128,
        "workers": 0,
        "learning_rate": 0.001,
    ...
    for epoch_index in range(100):
        ...
        train = frozen_impl.run_epoch(model, train_loader, device, optimizer)
        guard1 = model.stability_guard_activations
        reset = epoch_index in (50, 80)
        if reset:
            reset_audits.append({
                "epoch": epoch_index + 1,
                **frozen_impl.reset_prototypes_in_place(model, train_loader, device, optimizer),
            })
        val = frozen_impl.run_epoch(model, val_loader, device, None)
        guard2 = model.stability_guard_activations
        score = frozen_impl.validation_composite(val["accuracy"], val["macro_f1"])
    ...
    truth, predictions = frozen_impl.validation_predictions(model, input_dir, device)
    diagnostics = write_diagnostics(frozen_impl, args.protocol_id, run_dir, truth, predictions, input_dir)
    ...
    result = {
        ...
        "training_seed": training_seed,
        "known_classes": protocol["known_applications"],
        "unknown_classes": protocol["unknown_applications"],
        "train_samples": len(train_ds),
        "validation_samples": len(val_ds),
