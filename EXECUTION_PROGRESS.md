...
- 状态：`blocked_on_github_authentication`。
...
- 推送状态：尚未推送。用户已要求重试；HTTPS push 实际停在 GitHub 用户名提示，当前 `gh auth status` 未登录、SSH BatchMode 认证失败，远端 `main` 仍为 `25cd43c`。
- 安全下一步：用户完成 GitHub CLI 登录或配置 SSH key 后，在项目目录执行 `git push origin main`，随后核对远端分支 SHA；不得使用 force push。
