#!/usr/bin/env python3
"""Aggregate all 15 paired kNN diagnostics and apply the preregistered gate."""
...
def main() -> None:
    ...
    for scenario in ("a1", "a2", "a3"):
        for fold, seed in enumerate(config["seeds"]):
            ...
            if not (run_dir / "SUCCESS").is_file():
                raise RuntimeError(f"Incomplete Stage13A-1 run: {run_dir}")
    ...
    knn10 = [delta_row(result, "KNN_10", "CENTROID") for result in results]
    knn5 = [delta_row(result, "KNN_5", "CENTROID") for result in results]
    ...
    gate = apply_gate(knn10, knn5, config)
    SUMMARY.mkdir(parents=True, exist_ok=True)
    write_csv(SUMMARY / "run_level_results.csv", run_rows)
    write_csv(SUMMARY / "scenario_summary.csv", scenario_rows)
    write_csv(SUMMARY / "knn10_vs_centroid.csv", knn10)
    write_csv(SUMMARY / "knn5_sensitivity.csv", knn5)
    ...
    for scenario in ("a1", "a2", "a3"):
        value = gate["primary_characterization"]["scenarios"][scenario]
        lines.append(
    ...
    lines.extend([
        ...
        *[f"- {name}: `{'PASS' if passed else 'FAIL'}`" for name, passed in gate["conditions"].items()],
        "",
        f"- More stable sensitivity setting: `{gate['more_stable_k']}`.",
        f"- Fusion study worthwhile: `{gate['fusion_study_worthwhile']}`.",
