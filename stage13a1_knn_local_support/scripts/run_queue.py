#!/usr/bin/env python3
"""Run a deterministic shard of the 15 CPU-only Stage13A-1 evaluations."""
...
from stage13_common import CONFIG_PATH, STAGE_ROOT, read_json
...
def main() -> None:
    ...
    for number, (scenario, fold, seed) in enumerate(selected, start=1):
        print(f"stage13a1_progress={number}/{len(selected)} scenario={scenario} fold={fold} seed={seed}", flush=True)
        subprocess.run(
            [
                sys.executable,
                "-B",
